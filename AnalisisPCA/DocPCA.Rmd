...
El conjunto de datos sobre isotopos de carbono es un conjunto de datos multivariados de valor real que describe la criticidad (*K*) de un reactor y la potencia del mismo (*LP*).

Hay ciento doce columnas. Las primeras 110 se refieren a isotopos de carbono, donde *C* hace referencia al Carbono, el número después de la *C* a la cantidad de protones y neutrones en su núcleo, *U* a que es *inestable*, y *G* a que es emisor de *radiación gamma*. Por último, la columna *K* se refiere a la criticidad del reactor, y la columna *LP* a la potencia de este.
...
Primero, se descartan las columnas *K* y *LP* por no ser relevantes.
...
Debido a que ciertas columnas contenían el mismo valor en todos sus registros, el resultado de la estandarización es nulo, por lo tanto.
...
De forma geométrica podemos ver que los componentes principales representan las direcciones de los datos que explican la máxima cantidad de varianza. En el caso del primer componente se aprecia que en, la gráfica las *líneas* que representatn a las variables originales tienden a agruparse en el eje *x*, correspondiente a la *dimensión 1*.
